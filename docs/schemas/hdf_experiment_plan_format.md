...
- highlight the pump display's 16-character limit in the plan table (`highlight_pump_display_limit`,
  legacy table only - additive field, same defaulting behavior; GUI-only, no effect on what is sent
  to hardware, and always `False` when `show_on_pump_display` is `False`)
- valve state
- switch position
- per-channel flow
