...
def tabler_icon(*names: str) -> QIcon:
    for name in names:
        try:
            return TablerQIcon().get_qicon(name)
        except Exception:
            continue
    return QIcon()
...
def flow_tabler_icon(*names: str) -> QIcon:
    ...
    for name in names:
        ...
        for candidate in candidates:
            try:
                return TablerQIcon().get_qicon(candidate)
            except Exception:
                continue
    return QIcon()
...
def transport_icon(theme_mode: str, kind: str) -> QIcon:
    ...
    icon_map = {
        ...
        "previous": TablerQIcon.player_skip_back,
        "next": TablerQIcon.player_skip_forward,
        "stop_all": TablerQIcon.hand_stop,
        "status": TablerQIcon.reload,
    }
    icon = icon_map.get(kind, TablerQIcon.player_play)
