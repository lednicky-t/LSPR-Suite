"""Plan-table column-width/layout helpers, plus the real model construction
entry point (`configure_experiment_control_plan_table`).

Moved from sLSPR acq's `gui/experiment_control_table.py` (Phase 2, Tier 3a -
full experiment-control panel consolidation, 2026-08-09). `DeviceLifecycleController`/
`SELECTOR` and `flow_tabler_icon`/`tint_tabler_icon` were already fully shared
(`lspr_acq_shell.device_lifecycle`, `lspr_ui`) before this move - only the
import paths needed repointing.
"""
...
from .device_lifecycle import DeviceLifecycleController
from .device_types import SELECTOR
from .pump_plan import ACTIVE_PUMP_CHANNELS
from .experiment_control_plan_view import (
...
from .experiment_plan_table_model import ExperimentPlanTableModel
...
def configure_experiment_control_plan_table(window, *, app_name: str, app_version: str) -> ExperimentPlanTableModel:
    table = window.plan_table
